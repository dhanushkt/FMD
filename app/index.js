...
const loaderEle = document.getElementById('loading-model');
const demosSection = document.getElementById('demos');
const login_form = document.getElementById('login-form');
const live_cam_view = document.getElementById('live-view');
const webcambutton = document.getElementById('webcamButton');
//global variable to stop the video
var stopVideo = false;
...
async function loadMaskDetectionModel() {
  ...
  await tf.loadLayersModel(web_url_for_model + 'model.json').then(m => {
    ...
    //remove loader and enable camera once model is loaded
    loaderEle.classList.add('removed');
    live_cam_view.classList.remove('removed');
    ...
    for (let i = 0; i < faces.length; i++) {
      ...
      if (predictions.length > 0) {
        if (predictions[0] > decisionThreshold) {
          ...
          //face mask successfully detected
          faceMaskDetected();
        } else {
...
function pretrainedmodel() {
  ...
  loaderEle.classList.remove('removed');
  main();

}

function performLogin() {
  login_form.classList.add('removed');
  demosSection.classList.remove('removed');
}

function faceMaskDetected() {
  successEle = document.getElementById('mask-detected');
  successEle.classList.remove('hide-field');
  setTimeout(function () {
    stopVideo = true;
    demosSection.classList.add('removed');
